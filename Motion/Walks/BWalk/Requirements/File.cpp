...
{
  assert(!eof());
  assert(fread(p, 1, size, (FILE*)stream) > 0);
}
...
void File::write(const void* p, unsigned size)
{
  ...
  assert(fwrite(p, 1, size, (FILE*)stream) == size);
}
...
bool File::eof()
{
  ...
  else
  {
    ...
    {
      assert(ungetc(c, (FILE*)stream) != EOF);
      return false;
    }
  }
}
...
unsigned File::getSize()
{
  ...
  assert(fseek((FILE*)stream, currentPos, SEEK_SET) == 0);
...
const char* File::getBHDir()
{
  ...
  if(!dir[0])
  {
    ...
    {
#if defined(TARGET_SIM) || defined(TARGET_TOOL)
      assert(getcwd(dir, sizeof(dir) - 7) != 0);
