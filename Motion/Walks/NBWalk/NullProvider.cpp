...
NullProvider::NullProvider(boost::shared_ptr<Sensors> s, const bool chain_mask[Kinematics::NUM_CHAINS])
...
void NullProvider::calculateNextJointsAndStiffnesses(){
    ...
    std::vector<float> curMotionAngles = sensors->getBodyAngles();

    for(unsigned int chain = 0; chain < Kinematics::NUM_CHAINS; chain++){
        if( !chainMask[chain] )
            continue;

        //Iterate through the 22 long std::list of stiff/joints and
        //copy them into chain specific std::vectors to be sent to
