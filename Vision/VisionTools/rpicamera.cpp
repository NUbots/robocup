...
void RPiCamera::openCameraDevice(std::string device_name)
{
    ...
    if(fd != -1)
    {
        debug << "RPiCamera::RPiCamera(): " << device_name << " Opened Successfully." << endl;
    }
    else {
        debug << "RPiCamera::RPiCamera(): " << device_name << " Could Not Be Opened: " << strerror(errno) << endl;
    }
    #endif
}
...
void RPiCamera::setStreaming(bool streaming_on)
{
    ...
    debug << "RPiCamera: streaming - " << streaming_on << endl;
}
...
void RPiCamera::initialiseCamera()
{
    ...
    if(returnValue)
    {
        debug << "RPiCamera::RPiCamera(): Error Setting Video Mode: " << strerror(errno) << endl;
    }
    else
    {
        debug << "RPiCamera::RPiCamera(): Video Mode set to " << (WIDTH == 320 ? "QVGA" : "VGA") << endl;
    }
    #else
    VERIFY(!returnValue);
    #endif
    ...
    if(returnValue)
    {
        debug << "RPiCamera::RPiCamera(): Error Setting Format: " << strerror(errno) << endl;
    }
    else
    {
        debug << "RPiCamera::RPiCamera(): Format set" << endl;
    }
    #else
    VERIFY(!returnValue);
    #endif
...
void RPiCamera::loadCameraOffset()
{
    ...
    {
        errorlog << "RPiCamera::loadCameraOffset(). Unable to load camera offset." << endl;
    }
}
