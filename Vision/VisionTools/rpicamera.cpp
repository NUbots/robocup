...
storedTimeStamp(Platform->getTime())
{
    ...
    debug << "RPiCamera::RPiCamera()" << std::endl;
#endif
...
void RPiCamera::openCameraDevice(std::string device_name)
{
    // open device
    ...
    if(fd != -1)
    {
        debug << "RPiCamera::RPiCamera(): " << device_name << " Opened Successfully." << std::endl;
    }
    else {
        debug << "RPiCamera::RPiCamera(): " << device_name << " Could Not Be Opened: " << strerror(errno) << std::endl;
    }
    #endif
}

void RPiCamera::setStreaming(bool streaming_on)
{
    int type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    int instruction = streaming_on ? VIDIOC_STREAMON: VIDIOC_STREAMOFF;
    VERIFY(ioctl(fd, instruction, &type) != -1);
    debug << "RPiCamera: streaming - " << streaming_on << std::endl;
}

void RPiCamera::initialiseCamera()
{
    int returnValue;
    ...
    if(returnValue)
    {
        debug << "RPiCamera::RPiCamera(): Error Setting Video Mode: " << strerror(errno) << std::endl;
    }
    else
    {
        debug << "RPiCamera::RPiCamera(): Video Mode set to " << (WIDTH == 320 ? "QVGA" : "VGA") << std::endl;
    }
    #else
    VERIFY(!returnValue);
    #endif
    ...
    if(returnValue)
    {
        debug << "RPiCamera::RPiCamera(): Error Setting Format: " << strerror(errno) << std::endl;
    }
    else
    {
        debug << "RPiCamera::RPiCamera(): Format set" << std::endl;
    }
    #else
    VERIFY(!returnValue);
    #endif
...
const unsigned char* RPiCamera::getImage() const
{
  ASSERT(currentBuf);
  return static_cast<unsigned char*>(mem[currentBuf->index]);
}

double RPiCamera::getTimeStamp() const
{
  ASSERT(currentBuf);
  return timeStamp;
}
...
void RPiCamera::readCameraSettings()
{
    ...
    debug << "RPiCamera::readCameraSettings()" << std::endl;
...
int RPiCamera::readSetting(unsigned int id)
{
...
void RPiCamera::applySettings(const CameraSettings& newset)
{
    // AutoSettings
...
void RPiCamera::forceApplySettings(const CameraSettings& newset)
{
#if DEBUG_NUCAMERA_VERBOSITY > 1
...
void RPiCamera::loadCameraOffset()
{
    ...
    {
        errorlog << "RPiCamera::loadCameraOffset(). Unable to load camera offset." << std::endl;
    }
}
