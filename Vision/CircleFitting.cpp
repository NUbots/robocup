...
/*
Circle CircleFitting::FitCircleLMF(uint8* image, Blob* ballBlob, int direction) {

  cImage = image;

  numFittedPoints=0;

//  FindPointsOutwards(ballBlob,c_BALL_ORANGE,direction); // Finds the points on the  edge of the ball

  FindPointsInwards(ballBlob,c_ORANGE,direction); // Finds the points on the  edge of the ball
...
Circle CircleFitting::AlgebraicCircleFit() {
  ...
  if ((G22 < DBL_MIN) || (G22 > DBL_MAX))
    ...
    algebraicCircle.centreX = B/2 + meanX; // Sets the x coordinate of the centre of the circle

    algebraicCircle.centreY = C/2 + meanY; // Sets the y coordinate of the centre of the circle

    algebraicCircle.radius = sqrt((algebraicCircle.centreX-meanX)*(algebraicCircle.centreX-meanX)+(algebraicCircle.centreY-meanY)*(algebraicCircle.centreY-meanY)+meanXX+meanYY); // Determine the radius of the centr of the circle
