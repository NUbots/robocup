...
bool FieldEdgeDetection::ransacLine(
      const vector<pair<int, int> >& points, Line& result,
      pair<int, int>& resultp1, pair<int, int>& resultp2,
      bool **con, int k, float e, int n, unsigned int seed)
{
   ...
   for (int i = 0; i < k; ++i) {
      ...
      int concensus = 0;
      int q;
      vector<pair<int, int> >::const_iterator j;
