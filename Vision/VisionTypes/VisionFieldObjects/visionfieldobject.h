...
public:
	...
	const GroundPoint& getLocation() const
	{
		return m_location;
	}
	...
	Vector2<double> getScreenSize() const
	{
		return m_size_on_screen;
	}
	//! @brief returns the field position relative to the robot.

	virtual Vector3<double> getRelativeFieldCoords() const
	{
		return m_location.relativeRadial;
	}
