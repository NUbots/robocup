...
DarwinWebotsActionators::DarwinWebotsActionators(DarwinWebotsPlatform* platform) : m_simulation_step(int(platform->getBasicTimeStep()))
{
    ...
    debug << "DarwinWebotsActionators::DarwinWebotsActionators(). Avaliable Actionators: " << std::endl;
...
{
#if DEBUG_NUACTIONATORS_VERBOSITY > 3
    debug << "DarwinWebotsActionators::copyToHardwareCommunications()" << std::endl;
#endif
#if DEBUG_NUACTIONATORS_VERBOSITY > 4
    m_data->summaryTo(debug);
#endif
    ...
    copyToLeds();
	#if DEBUG_NUACTIONATORS_VERBOSITY > 0
    	debug << "DarwinWebotsActionators::copyToHardwareCommunications():Copying to sound." << std::endl;
	#endif
    copyToSound();
	#if DEBUG_NUACTIONATORS_VERBOSITY > 0
    	debug << "DarwinWebotsActionators::copyToHardwareCommunications():Copying to teleporter." << std::endl;
	#endif
    copyToTeleporter();
}
...
void DarwinWebotsActionators::copyToLeds()
{
	...
	#if DEBUG_NUACTIONATORS_VERBOSITY > 4
    	debug << "DarwinWebotsActionators::copyToLeds():Getting LED values." << std::endl;
    #endif
    ...
    {
		#if DEBUG_NUACTIONATORS_VERBOSITY > 4
    		debug << "DarwinWebotsActionators::copyToLeds():Copying LED #" << i << "." << std::endl;
		#endif
        unsigned char bgr[3];
