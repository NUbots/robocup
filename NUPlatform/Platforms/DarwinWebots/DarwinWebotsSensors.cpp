...
DarwinWebotsSensors::DarwinWebotsSensors(DarwinWebotsPlatform* platform) : m_simulation_step(int(platform->getBasicTimeStep()))
{
    ...
    debug << "DarwinWebotsSensors::DarwinWebotsSensors()" << endl;
#endif
    m_platform = platform;
...
{
#if DEBUG_NUSENSORS_VERBOSITY > 4
    debug << "DarwinWebotsSensors::copyFromHardwareCommunications()" << endl;
#endif
    copyFromJoints();
...
{   
	#if DEBUG_NUSENSORS_VERBOSITY > 4
        debug << "DarwinWebotsSensors::copyFromJoints()" << endl;
    #endif
...
#if DEBUG_NUSENSORS_VERBOSITY > 4
    debug << "DarwinWebotsSensors::copyFromAccelerometerAndGyro()" << endl;
#endif
...
#if DEBUG_NUSENSORS_VERBOSITY > 4
    debug << "DarwinWebotsSensors::copyFromDistance()" << endl;
#endif
...
#if DEBUG_NUSENSORS_VERBOSITY > 4
    debug << "DarwinWebotsSensors::copyFromFootSole()" << endl;
#endif
...
{
    #if DEBUG_NUSENSORS_VERBOSITY > 4
        debug << "DarwinWebotsSensors::copyFromFootBumper()" << endl;
    #endif
...
void DarwinWebotsSensors::copyFromGPS()
{
    ...
    if (m_gps != NULL)
    {
        ...
        #if DEBUG_NUSENSORS_VERBOSITY > 4
            debug << "DarwinWebotsSensors::copyFromGPS()" << endl;
        #endif
...
void DarwinWebotsSensors::copyFromCompass()
{
    if (m_compass != NULL)
    {
        ...
        #if DEBUG_NUSENSORS_VERBOSITY > 4
            debug << "DarwinWebotsSensors::copyFromCompass()" << endl;
        #endif
