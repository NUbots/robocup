...
bool WeightedModel::operator ==(const WeightedModel& b) const
{
    if(m_active != b.m_active) return false;
    if(m_alpha != b.m_alpha) return false;
    if(m_id != b.m_id) return false;
    if(m_parent_id != b.m_parent_id) return false;
    if(m_creation_time != b.m_creation_time) return false;
    //if(m_parent_history_buffer != b.m_parent_history_buffer) return false;
    return true;
}
