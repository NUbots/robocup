...
protected:
    ...
    void doState()
    {
        ...
        /*if (m_data->CurrentTime >= m_pan_end_time+200) {
            ...
            debug << "GoToBall::doState() - checking kick" << endl;
        #endif
        ...
        {
            #if DEBUG_BEHAVIOUR_VERBOSITY > 2
                debug << "GoToBall::doState() - walking start" << endl;
            #endif
           ...
           // speed = BehaviourPotentials::goToBallDirectWithSidewardsKick(ball, self, BehaviourPotentials::getBearingToOpponentGoal(m_field_objects, m_game_info),targetKickDistance,37.);
            vector<float> result;
            #if DEBUG_BEHAVIOUR_VERBOSITY > 2
                debug << "GoToBall::doState() - adding walk job" << endl;
            #endif
            // if the ball is too far away to kick and the obstacle is closer than the ball we need to dodge!
            result = speed;
        ...
        {
            #if DEBUG_BEHAVIOUR_VERBOSITY > 2
                debug << "GoToBall::doState() - starting kick" << endl;
            #endif
            //m_jobs->addMotionJob(new WalkJob(0, 0, 0));
            Vector2<float> kickPosition(0,0);            
            Vector2<float> targetPosition(0,0);
            vector<float> _kickPosition(2,0);
            vector<float> _targetPosition(2,0);
            ...
            _kickPosition[0] = balldistance * cos(ballbearing);
            _kickPosition[1] = balldistance * sin(ballbearing);
            _targetPosition[0] = goalPosition[0] * cos(goalPosition[1]);
            _targetPosition[1] = goalPosition[0] * sin(goalPosition[1]);
            WalkKickJob* kjob = new WalkKickJob(0,kickPosition, targetPosition);
            #if DEBUG_BEHAVIOUR_VERBOSITY > 2
                debug << "GoToBall::doState() - adding kick motion job" << endl;
            #endif
            //KickJob* kjob = new KickJob(0,_kickPosition,_targetPosition);
