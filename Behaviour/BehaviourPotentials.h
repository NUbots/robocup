...
public:
    ...
    static vector<float> goToBall(MobileObject& ball, Self& self, float heading, float kickingdistance = 15.0, float stoppingdistance = 65)
    {
        ...
        {
            bearing = ball.estimatedBearing();
        }
        bool offset_sign = mathGeneral::sign(bearing);
        ...
        if(fabs(angle_to_kick_pos) > 0.5 and dist_to_kick_pos > stoppingdistance)
        {
            speed[0] = 0.05f; // 5% speed
            speed[1] = mathGeneral::PI;
            speed[2] = -0.5*angle_to_kick_pos;

//            std::cout << "Calculated heading: " << -atan2(y_diff, x_diff) << std::endl;
//            std::cout << "My heading: " << my_heading << std::endl;
//            std::cout << "Heading to Ball: " << bearing << std::endl;
