...
{
    //std::cout << "Sample ID, Start X, Start Y, Start Heading, End X, End Y, End Heading, Trans Speed, Trans Direction, Turn Speed, X-odometry, Y-odometry, Bearing-odometry, Time Taken" << std::endl << flush;
    samplePointIndex = 0;
    std::vector<float> walk = Sampling::HaltonPointND(samplePointIndex,3);
...
void SlipTestProvider::doBehaviour()
{
    ...
    hist_gpsY.push_back(self.wmY());
    hist_gpsB.push_back(self.Heading());
    //std::cout << "SlipTestOdometry: " << odometry[0] << ", " << odometry[1] << ", " << odometry[2] << ", " << self.wmX() << ", " << self.wmY() << ", " << self.Heading() << std::endl << flush;
    ...
    if (m_av_count == 50) {
        ...
        for (int i = 0; i < 3; i++) {
            std::cout << m_slip_matrix[i][0] << "  \t" << m_slip_matrix[i][1] << "  \t" << m_slip_matrix[i][2] << std::endl;
        }
    ...
    if (m_return_to_start) {
        ...
        float dpy = self.wmY();
        std::vector<float> fieldstate;
        fieldstate.push_back(150.);
        fieldstate.push_back(0.);
        fieldstate.push_back(0.);
        std::vector<float> walkdirection = BehaviourPotentials::goToFieldState(self,fieldstate);
        Blackboard->Jobs->addMotionJob(new WalkJob(walkdirection[0]/1.2,walkdirection[1],walkdirection[2]/2.));
        //std::cout << "going to start: " << dpx*dpx+dpy*dpy << std::endl << std::flush;
    ...
    //do a test run
        ...
        if (m_run_start < Blackboard->Sensors->CurrentTime-7000.) {
            ...
            //std::cout << samplePointIndex << ", " << m_start_x << ", " << m_start_y << ", " << m_start_heading << ", " << self.wmX() << ", " << self.wmY() << ", " << self.Heading() << ", " << m_trans_speed << ", " << m_trans_direction << ", " << m_turn_speed << ", " << m_odomX << ", " << m_odomY << ", " << m_odomB << ", " << Blackboard->Sensors->CurrentTime-m_run_start-2000. << std::endl << flush;
            m_return_to_start = true;
