...
ScriptTunerProvider::ScriptTunerProvider(Behaviour* manager, bool pauseable) : BehaviourFSMProvider(manager)
{
    m_paused_state = new ScriptTunerState(this);
    
    m_state = m_paused_state;
}


ScriptTunerProvider::~ScriptTunerProvider()
{
    delete m_paused_state;
}

BehaviourState* ScriptTunerProvider::nextStateCommons()
{
    return m_state;
}
