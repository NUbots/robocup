...
ChaseBallProvider::ChaseBallProvider(Behaviour* manager, bool pauseable) : BehaviourProvider(manager)
{
...
void ChaseBallProvider::doBehaviour()
{
    if (m_data->CurrentTime < 500)
        return;
    
    // Handle the pausing on chest press
    if (m_pauseable and (singleChestClick() or longChestClick()))
    {
        ...
        if (m_paused)
        {
            ...
            m_jobs->addMotionJob(new HeadJob(m_current_time + 500, zero));
        }
    }
...
void ChaseBallProvider::TrackPoint(float sensoryaw, float sensorpitch, float elevation, float bearing, float centreelevation, float centrebearing)
{
